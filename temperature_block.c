...
void populate_R_model_block(block_model_t *model, flp_t *flp)
{
	...
	model->r_ready = TRUE;
	FILE *fpr;
	fpr=fopen("Gmatrix_block", "wa");
	printf("NL*n+EXTRA = %d*%d+%d\n", NL,n,EXTRA);
  	for (i=0; i < NL*n+EXTRA; i++) {
      for (j=0; j < NL*n+EXTRA; j++) {
          fprintf(fpr, "%f\t", b[i][j]);
      }
...
void set_internal_power_block(block_model_t *model, double *power)
{
	...
	FILE *fpc;
    fpc=fopen("Pmatrix_block", "wa");
    for (i=0; i < NL*model->n_units+EXTRA; i++) {
        fprintf(fpc, "%f\t", power[i]);
        fprintf(fpc, "\n");
    }
    fclose(fpc);
}
