...
// void export_temp_grid(grid_model_t *model, grid_model_vector_t *temp)
// {
//   int i, j, n;
...
//   FILE *fp = fopen("grid_temp", "w");
//   if (fp == NULL)
//     fatal("cannot open grid_temp file\n");

//   for (n = 0; n < model->n_layers; n++)
//     for (i = 0; i < model->rows; i++)
//       for (j = 0; j < model->cols; j++)
//         fprintf(fp, "%.6f\n", temp->cuboid[n][i][j] - 273.15);
...
void compute_temp_grid(grid_model_t *model, double *power, double *temp, double time_elapsed)
{
  ...
  // export_temp_grid(model, model->last_trans);
...
// export the occupancy added by syc
// void export_blist(grid_model_t *model)
// {
//   int l,i, j;
//   FILE *fp = fopen("occupancy", "w");
//   if (fp == NULL)
//     fatal("cannot open occupancy file\n");

//   for(l=0; l < model->n_layers; l++)
//     for(i=0; i < model->rows; i++)
//       for(j=0; j < model->cols; j++) {
//         fprintf(fp, "%f\n", model->layers[l].b2gmap[i][j]->occupancy);
//       }
//   fclose(fp);
// }
